...
def prepare_pairwise_data(elo_df, metric_dfs, dataset_name):
    ...
    for idx, row in elo_df.iterrows():
        ...
        # Create difference vector: winner - loser
        winner_metrics = np.array(winner_metrics)
        loser_metrics = np.array(loser_metrics)
        delta = winner_metrics - loser_metrics
        
        X_delta.append(delta)
        y.append(1)  # Always 1 since winner should be preferred
        
        sample_data['winner_elo'] = row.get('winner', 0)
        sample_data['loser_elo'] = row.get('loser', 0) 
        sample_data['metrics'] = valid_metrics
